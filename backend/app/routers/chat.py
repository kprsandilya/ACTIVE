...
def load_whisper_model(model_name="openai/whisper-tiny") -> Pipeline:
    ...
    Uses cached model weights if available; otherwise, fails gracefully with instructions.
    """
    try:
...
async def handle_audio(file: UploadFile = File(...)):
    ...
    # Always translate to English using task="translate"
    result = stt(tmp_path, generate_kwargs={"task": "translate"})
    user_text = result["text"].strip()
    print(f"[STT] Transcribed text: {user_text}")

    assistant_reply = query_ollama(
        f"The user said: {user_text}\n"
        "You are an expert agricultural advisor. Respond with 1-2 sentences focusing on helping the farmer save money."
    )
    return {"reply": assistant_reply}
